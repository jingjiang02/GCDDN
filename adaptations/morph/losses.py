...
def onehot_label(label, num_class, ignore_label=None):
    label = label.detach().clone()
    if ignore_label is not None:
        num_class = num_class + 1
        if ignore_label == -1:
            label = label + 1
        elif ignore_label == 255:
            label[label == ignore_label] = num_class - 1
        else:
            raise Exception
...
def dice_loss_one_hot(y_true, y_pred, ignore_label=None):
    eps = 1e-8
    ndims = len(y_pred.shape) - 2
    vol_axes = list(range(2, ndims + 2))
    top = 2 * (y_true * y_pred).sum(dim=vol_axes)
    bottom = (y_true + y_pred).sum(dim=vol_axes)
    num_class = top.shape[-1]

    if ignore_label is not None:
        if ignore_label < 0:
            ignore_label = 0
        elif ignore_label >= num_class:
            ignore_label = num_class - 1
        else:
            raise Exception

        mask = torch.ones(num_class, dtype=torch.bool, device=y_true.device)
        mask[ignore_label] = False
        top = top[:, mask]
        bottom = bottom[:, mask]

    dice = torch.mean(top / (bottom + eps))
    return 1 - dice


def dice_loss(y_true, y_pred, weight=None, return_per_loss=False, ignore_label=None):
...
class SSIM_loss(torch.nn.Module):
    def __init__(self, data_range=255, if_MS=True, win_size=11, channel=1):
        super(SSIM_loss, self).__init__()
        if if_MS:
            self.SSIM = MS_SSIM(win_size=win_size, data_range=data_range, size_average=True, channel=channel)
        else:
            self.SSIM = SSIM(data_range=data_range, size_average=True, channel=channel)
