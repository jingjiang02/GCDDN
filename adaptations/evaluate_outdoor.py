...
from compute_iou import fast_hist, per_class_iu, SegmentationMetric
...
def main():
    ...
    if args.model == 'Trans4PASS_plus_v2':
        model = Trans4PASS_plus_v2(num_classes=args.num_classes, norm=norm)
    else:
        raise Exception
    ...
    targettestset = densepass13TestDataSet(args.data_dir, args.data_list, crop_size=(w, h),
                                           mean=IMG_MEAN,
                                           scale=False, mirror=False, set='val', normalize=False, org_mapping=True)
